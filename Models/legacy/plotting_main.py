...
def tune_hyperparameters(asc_data, desc_data, rainfall_data):
    ...
    for window_size in range(5, 7, 1):  # Test window sizes from 5 to 30 (step 2)
        for threshold in np.arange(0.05, 0.2, 0.05):  # Test thresholds from 0.05 to 0.7 (step 0.05)
...
def main():
    ...
    import numpy.ma as ma  # For masking non-risk areas

    # Reduce risk values by 20%, ensuring no negatives
    risk_map_adjusted = np.maximum(risk_map - 20, 0)

    # Define a **smooth green gradient** colormap (White → Light Green → Dark Green)
    colors = ["#ffffff", "#d4f7c5", "#a0e18e", "#4db560", "#006400"]  # White to deep green
    cmap = LinearSegmentedColormap.from_list("custom_green", colors, N=256)  # 256 levels for smooth transition

    # Mask non-risk areas (force them transparent so background shows)
    risk_map_masked = ma.masked_where(risk_map_adjusted == 0, risk_map_adjusted)
    ...
    # Get the current axis and **set true light blue background**
    ax = plt.gca()
    ax.set_facecolor("#b3e0ff")  # ✅ Light blue for water areas (non-risk regions)

    # Plot the **masked risk map** so only land areas get green gradient
    im = plt.imshow(risk_map_masked, cmap=cmap, norm=norm, interpolation="nearest")  # No smoothing, true risk values
    ...
    plt.title("Smooth Landslide Risk Map (Green Gradient)", fontsize=14, fontweight="bold")
    plt.xlabel("Longitude", fontsize=12)
    plt.ylabel("Latitude", fontsize=12)
    plt.grid(visible=True, color="white", linestyle="--", linewidth=0.5)

    # Save the enhanced risk map
    output_png = os.path.join(OUTPUT_DIR, "smooth_continuous_green_risk_map.png")
    plt.savefig(output_png, dpi=300, bbox_inches="tight", facecolor="lightgray")
    plt.close()
    print(f"[Main] Smooth continus gradient risk map saved to: {output_png}")
    print(risk_map)  # Print the raw numpy array

        # Save the raw numpy array to a file
    output_npy = os.path.join(OUTPUT_DIR, "riskg_map_probabilities.npy")
    ...
    output_txt = os.path.join(OUTPUT_DIR, "riskg_map_probabilities.txt")
