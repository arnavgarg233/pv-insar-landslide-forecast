"""
Model H: RF with 3 Statistical Features + Rainfall (Fewer features)
Tests how RF performs with fewer features
"""
...
print("="*70)
print("MODEL H: RF with 3 Statistical Features + Rainfall")
print("="*70)
...
# Extract only 3 statistical features
print("\n[2/4] Extracting 3 statistical features...")
time_steps, height, width = asc_data.shape
combined_data = (asc_data + desc_data) / 2
combined_flat = combined_data.reshape(time_steps, -1)

# Use top 3 most important from Model F: Range, Min, Max
...
stat_features = np.column_stack([
    range_per_time, min_per_time, max_per_time
])

print(f"  Extracted {stat_features.shape[1]} statistical features")
print(f"  Features: Range, Min, Max (top 3 from Model F)")
...
# Combine features: 3 stats + 1 rainfall = 4 total
all_features = np.hstack([stat_features, aligned_rainfall.reshape(-1, 1)])

print(f"  Total features: {all_features.shape[1]} (3 stats + 1 rainfall)")
...
print("Model G (RF - 6 stats + rain):        Accuracy: 91.67%, F1: 93.62%, AUC: 96.10%")
print(f"Model H (RF - 3 stats + rain):        Accuracy: {acc*100:.2f}%, F1: {f1*100:.2f}%, AUC: {auc*100:.2f}%")
print("="*70)

# Show feature importances
importances = rf.feature_importances_
feature_names = ['Range', 'Min', 'Max', 'Rainfall']

print("\n" + "="*70)
print("FEATURE IMPORTANCES:")
print("="*70)
for i, name in enumerate(feature_names):
    print(f"  {i+1}. {name}: {importances[i]:.4f}")
print("="*70)
